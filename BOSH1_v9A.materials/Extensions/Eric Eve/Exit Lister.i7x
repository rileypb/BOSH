...
The door appearance rules is a door based rulebook producing truth states.
The room appearance rules is a room based rulebook producing truth states.

Definition: a door is apparent:
	decide on the truth state produced by the door appearance rules for it;
...
Definition: a room is apparent:
	decide on the truth state produced by the room appearance rules for it;
...
A room appearance rule for a room in hyperspace (this is the hyperspace appearance rule):
	if the location is in hyperspace:
		make no decision;
	if the player is wearing the astral lenses:
		make no decision;
...
To decide what list of directions is the exit list:
  let result be a list of directions;
  let farplace be location;   
  repeat with way running through directions
  begin;
    let farplace be the room way from the location;
