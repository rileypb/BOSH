...
excuse text for a direction (called dir) (this is the standard excuse text rule):
	...
	if dir is down:
		if source is floored:			
			rule succeeds with result "[We] [can't] just dig downward." (B) in sentence case;
		otherwise:
...
The can't exit when not inside anything rule does nothing when 1 is 1. [work around: in 10.1.2, if this statement is not qualified, the response text will be considered non-existent]
