Image Strip by Philip Riley begins here.
...
[ RockSwitchingSGW;
	switch( (+current glulx rock+) )
	{
		...
		GG_HIDEWIN_ROCK: gg_hidewin = (+ current glulx rock-ref +);
	}
];
...
[ FindWidth  result graph_width;
 ...
 [ MyRedrawGraphicsWindows cur_pic_1 cur_pic_2 cur_pic_3 cur_pic_4 cur_pic_5 result graph_width graph_height pic_count
	...
	FollowRulebook((+glulx picture selection rules+));
	pic_count = latest_rule_result-->2;
	if (pic_count > 0) { 	cur_pic_1 = ResourceIDsOfFigures-->(+ internally selected picture 1 +); }   
	if (pic_count > 1) { 	cur_pic_2 = ResourceIDsOfFigures-->(+ internally selected picture 2 +); }   
	if (pic_count > 2) { 	cur_pic_3 = ResourceIDsOfFigures-->(+ internally selected picture 3 +); }   
	if (pic_count > 3) { 	cur_pic_4 = ResourceIDsOfFigures-->(+ internally selected picture 4 +); }   
	if (pic_count > 4) { 	cur_pic_5 = ResourceIDsOfFigures-->(+ internally selected picture 5 +); }   
...
[ BlankWindowToColor color result graph_width graph_height;
 ...
 [ TileFillGraphicsWindows cur_pic result graph_width graph_height 
	...
	if (FollowRulebook( (+glulx picture selection rules+) ) ) { cur_pic = ResourceIDsOfFigures-->(+ internally selected picture 1 +); }   
	if (cur_pic == 0) rtrue;
...
[ DrawPhoneHandset phone_handset_pic w_offset h_offset w_total h_total graph_width;
	if (gg_picwin) {
		phone_handset_pic = (+ Figure Phone Handset +) + 1;

		glk_window_get_size(gg_picwin, gg_arguments, gg_arguments+WORDSIZE);
		graph_width  = gg_arguments-->0;
		
		w_offset = 5;
		h_offset = 5 + graph_width * (+ handset count - 1+);
...
Currently shown picture 1 is a figure-name that varies. [This the author may set during the course of the source.]

Currently shown picture 2 is a figure-name that varies. [This the author may set during the course of the source.]

Currently shown picture 3 is a figure-name that varies. [This the author may set during the course of the source.]

Currently shown picture 4 is a figure-name that varies. [This the author may set during the course of the source.]

Currently shown picture 5 is a figure-name that varies. [This the author may set during the course of the source.]

Internally selected picture 1 is a figure-name that varies. [This is the picture selected by the picture selection rules, which might be the 'currently shown picture' (by default) or might be, say, the current frame of an animation in progress. The author should not set this directly, but instead write additional picture selection rules if he wants to change it.]

Internally selected picture 2 is a figure-name that varies. [This is the picture selected by the picture selection rules, which might be the 'currently shown picture' (by default) or might be, say, the current frame of an animation in progress. The author should not set this directly, but instead write additional picture selection rules if he wants to change it.]

Internally selected picture 3 is a figure-name that varies. [This is the picture selected by the picture selection rules, which might be the 'currently shown picture' (by default) or might be, say, the current frame of an animation in progress. The author should not set this directly, but instead write additional picture selection rules if he wants to change it.]

Internally selected picture 4 is a figure-name that varies. [This is the picture selected by the picture selection rules, which might be the 'currently shown picture' (by default) or might be, say, the current frame of an animation in progress. The author should not set this directly, but instead write additional picture selection rules if he wants to change it.]

Internally selected picture 5 is a figure-name that varies. [This is the picture selected by the picture selection rules, which might be the 'currently shown picture' (by default) or might be, say, the current frame of an animation in progress. The author should not set this directly, but instead write additional picture selection rules if he wants to change it.]
...
The glulx picture selection rules are a rulebook producing numbers.

A glulx picture selection rule (this is the default picture selection rule):
	now the internally selected picture 1 is the currently shown picture 1;
	now the internally selected picture 2 is the currently shown picture 2;
	now the internally selected picture 3 is the currently shown picture 3;
...
The window resizing rules are a rulebook.
