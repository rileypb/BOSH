...
Include Texting by Philip Riley.
Include Computers by Philip Riley.
...
Petula Goldberg is a woman in the Room of Stuff. The description is "[regarding Petula Goldberg]Agent [we] [have] lots of dogs, a fact which can be inferred by the inevitable fur somewhere on [our] person. The last temp worker at the Bureau left due to an extreme allergic reaction to [Goldberg].".
...
When play begins: 
	...
	say "'First name, last name, gender?'[paragraph break]";  
	say "(1) Ezra Gaunt (male), a skeptical, quick-witted, yet secretly nervous and insecure man and winner of the 2006 Baltimore County Edgar Allan Poe Trivia Competition;[paragraph break]";
	say "(2) Petula Clark Goldberg (female), a skeptical, quick-witted, yet secretly nervous and insecure woman and former dog trainer;[paragraph break]";
	say "or (3) Larch Faraji (non-conforming), a skeptical, quick-witted, yet secretly nervous and insecure person whose apartment has a whole bedroom for snake cages.";
...
To text klimp something he already knows:	
	say text "yeh i know already";
...
Response of Klimp when texted asking about the Bureau of Strange Happenings:
	Say text "civs only hope (IMHO LOL)";
...
Rule for writing a paragraph about Margaret the temp when the location is the front office:
	say "The Bureau's latest temporary worker, [Margaret] is bustling around with unrestrained enthusiasm, unpacking boxes and assembling an approximation of a respectable front office, all as if the Bureau isn't going to be shut down before the year is out";
	if the front office is not visited:
		say ". [They] [see] [player's surname] and practically [squeal]. '[player's forename]! I'm sorry -- [agent]! How are you? I hope you don't mind -- I took the liberty of putting your desk together. I just love hex wrenches, don't you? Also, your new phone is here. I put it in your desk. Good timing! Now I just need you to give me your old phone so I can deactivate it.'[paragraph break]";
	...
	otherwise:
		...
		if the current interlocutor is not a person, now the current interlocutor is margaret the temp;
		
The how-are-you-reply of Margaret the temp is "I'm great! Thanks!".
...
The drawing of the Pitfall snake is scenery in biff's office. Understand "video/game/cobra/darius" and "darius 5200" as drawing of the pitfall snake. "Pencil on canvas, it's a fuzzy drawing of the 8-bit snake in the classic video game Pitfall."
...
Margaret the temp is in The Front Office.  
...
Klimp's Computer is a computer on the chief's desk. It is scenery.
...
driveway	front step	"[We] [mount] the steps to the front porch."
...
Margaret the temp has portrait Figure Margaret.
...
To decide which figure name is the picture of the location:
	...
	if location is regionally in (Marsh Street):
		now result is Figure House;
		...
		if current play mode is _normal:
			...
			repeat with C running through people talking with the player:
				if the portrait of C is not listed in characters:
					add the portrait of C to characters;
			if car 1 is in the location:
				add Figure Car to characters;
...
The story is always "Our protagonist Biff receives a call on his mobile phone. On the other end is a mysterious voice (who turns out later to be Doris). The voice tells him to pay attention to the story of Draco Fryes, as there is a scared little boy out there somewhere. Biff should check out 62 Marsh Street, home of Senator Fryes' son and grandson.

Biff goes to Marsh Street. After some trouble, he locates young Draco in the attic. Draco is, to Biff's surprise, an immature lizard man. He had been living in disguise with the Fryes as their son. (He was disguised how?) 

After a conversation, the two descend to the first floor where they are confronted by a heavy with a gun. Draco uses some kind of mind power to subdue the man. Draco and Biff escape to the basement where Draco is looking for something.

Biff finds a secret exit. It exits onto a tunnel that takes them to a futuristic subway system. After some trouble, they find a secret outpost of some future rebel group. Doris meets them there. 

After thanking Biff for finding Draco, Doris puts Biff to sleep.

The story resumes some weeks later, during a blackout. After the lights are back on, there is something on Biff's desk.

"

Book 1 - Puzzles at 62 Marsh Street

The puzzles are always "Get into the house by buying bolt cutters at the convenience store and cutting the padlock. You also need a flashlight, which is located in the laundromat. You must open the attic door. It doesn't have a pull chain, though -- it is operated by a button on the wall. But the electricity is out.  The electric panel is in the basement, which is locked. The lock is openable by a key hidden in plain sight - a funny shaped knife or fork for instance."

Book 2 - The Thumb Drive

The thumb-drive-story is always "Biff found the thumb drive in an excavated cave once inhabited by the Onondaga tribe, on a farm in upstate New York. He was never able to read it.".
...
Message-on-the-thumbdrive is always "They found me out and sent me back to, I think, the 14th century. Savra is going to move on the Dragon. Must save him.".
